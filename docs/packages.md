...
| [`benzene-grpc`](https://pypi.org/project/benzene-grpc/) | `benzene.grpc` | the Benzene↔gRPC status mapping + `benzene-status` trailer rule, and the gRPC server/client transport, including structured errors on `grpc-status-details-bin` (the `[transport]` extra adds `grpcio` + `grpcio-status`) | `benzene-core` |
...
| [`benzene-azure`](https://pypi.org/project/benzene-azure/) | `benzene.azure` | the Azure Functions host (HTTP + Service Bus + Event Hub + Queue Storage + Blob Storage + Cosmos DB change feed + Timer + Event Grid inbound, Service Bus/Event Hub/Queue Storage/Event Grid outbound clients) | `benzene-core`, `benzene-http` |
