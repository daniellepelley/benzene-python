...
| [`benzene-grpc`](https://pypi.org/project/benzene-grpc/) | `benzene.grpc` | the Benzene↔gRPC status mapping + `benzene-status` trailer rule, and the gRPC server/client transport (the `[transport]` extra adds `grpcio`) | `benzene-core` |
