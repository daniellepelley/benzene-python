# EKS deployment for examples/k8s_mesh — the AWS leg of the Kubernetes mesh example, mirroring
# .NET's examples/K8sMesh/deploy conventions (S3 remote state, project-prefixed names, deployed by
# .github/workflows/deploy-k8s-mesh-eks.yml).
#
...
# UNCHANGED examples/k8s_mesh/k8s manifests, applied by the workflow via the deploy/eks kustomize
...
terraform {
  ...
  # Remote state in S3 so the state survives between (ephemeral) CI runs — same bucket the AWS mesh
  # example (deploy/mesh) uses, different key. Configured at `terraform init` time via -backend-config.
  backend "s3" {}
  required_providers {
    aws = {
