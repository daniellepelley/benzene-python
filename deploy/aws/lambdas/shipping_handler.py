"""AWS Lambda entrypoint for the **shipping** mesh service.

The Lambda container image is shared by all three services; each function's
``image_config.command`` points at this module's ``handler`` (i.e. ``shipping_handler.handler``).
