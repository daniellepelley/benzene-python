"""AWS Lambda entrypoint for the **orders** mesh service.

The Lambda container image is shared by all three services; each function's
``image_config.command`` points at this module's ``handler`` (i.e. ``orders_handler.handler``).
